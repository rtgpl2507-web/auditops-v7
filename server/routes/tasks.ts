...
taskRouter.get('/:framework', async (req: Request, res: Response) => {
...
taskRouter.post('/:framework', async (req: Request, res: Response) => {
  const { framework } = req.params as { framework: FrameworkType };
...
taskRouter.put('/:framework/:taskId', async (req: Request, res: Response) => {
...
taskRouter.delete('/:framework/:taskId', async (req: Request, res: Response) => {
  const { framework, taskId } = req.params as { framework: FrameworkType; taskId: string };
  try {
    const data = await getFrameworkData(framework);
    if (!data.tasks) data.tasks = [];
...
taskRouter.delete('/:framework/:taskId/documents/:fileId', async (req: Request, res: Response) => {
...
taskRouter.get('/:framework/export', async (req: Request, res: Response) => {
  const { framework } = req.params as { framework: FrameworkType };
  try {
    const data = await getFrameworkData(framework);
...
taskRouter.post('/:framework/import', csvUpload.single('file'), async (req: Request, res: Response) => {
  const { framework } = req.params as { framework: FrameworkType };
